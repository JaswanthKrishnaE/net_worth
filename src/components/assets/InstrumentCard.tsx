...
type Props = {
  item: Instrument;
  badge: string;
  overrideMetric?: "current" | "invested" | "returns" | "xirr";
};
export default function InstrumentCard({ item, badge, overrideMetric }: Props) {
  const colors = useTheme();

  const metrics = ["current", "invested", "returns", "xirr"] as const;
  ...
  const displayValue = useMemo(() => {
    switch (metric) {
      case "current":
        return `₹${item.currentValue.toLocaleString()}`;
      case "invested":
        return `₹${item.investedValue.toLocaleString()}`;
      case "returns":
        return `₹${(item.currentValue - item.investedValue).toLocaleString()}`;
      case "xirr":
        return `${item.xirr}%`;
    }
  }, [metric, item]);
